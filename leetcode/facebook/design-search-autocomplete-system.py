...
class AutocompleteSystem(object):
    ...
    def input(self, c):
        ...
        """
        if c == '#':
            self.add(''.join(self.inputted_chars), 1)
            self.inputted_chars = []
            return []

        self.inputted_chars.append(c)

        node = self.root
        for ch in self.inputted_chars:
            if ch not in node.children:
                return []
            node = node.children[ch]

        heap = []
        for sentence, times in node.times.items():
            heapq.heappush(heap, (0 - times, sentence))
