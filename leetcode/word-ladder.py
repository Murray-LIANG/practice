        ...
        while queue:
            ...
            for _ in range(queue_size):
                ...
                if currentWord == endWord:
                    return steps
                for nb in self.neighbors(wordList, currentWord):
                    if nb not in visited:
                        visited.append(nb)
                        queue.append(nb)
        return 0

    def neighbors(self, wordList, word):
        res = []
        word = list(word)
...
# Solution 1 will exceed time limit.
